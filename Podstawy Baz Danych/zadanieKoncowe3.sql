...
group by [Order Details].OrderID, Orders.CustomerID,Orders.OrderID;
...
--3
	--a
	select a.FirstName, a.LastName, count(distinct o.OrderID) as total_orders
	from Employees as a 
	...
	select a.FirstName, a.LastName, sum(od.unitprice * od.quantity * (1- od.discount)) as total_price
	from Employees as a 
		join Orders as o on a.EmployeeID = o.EmployeeID
	...
	from Employees as a 
		join Orders as o on a.EmployeeID = o.EmployeeID
	group by a.EmployeeID,a.FirstName, a.LastName
	having a.EmployeeID in (select distinct reportsto from Employees)
	order by total_orders desc
	...
	from Employees as a 
		join Orders as o on a.EmployeeID = o.EmployeeID
	group by a.EmployeeID,a.FirstName, a.LastName
	having a.EmployeeID not in (select distinct reportsto from Employees where reportsto is not null)
	order by total_orders desc;
